...
Read the JIRA Personal Access Token and Base URL:
...
Read the acceptance criteria custom field ID (configurable, defaults to `customfield_10208`):
...
| Acceptance Criteria | `fields.${AC_CUSTOM_FIELD}` | Custom field ID — configurable via `AC_CUSTOM_FIELD` in `local.config` (default: `customfield_10208`) |
...
| 5 | **All FRs have sources** | §4.5 Functional Requirements | Every FR row has a non-empty Source column tracing back to JIRA, acceptance criteria, or an POD-spec section. | Add the missing source reference. |
