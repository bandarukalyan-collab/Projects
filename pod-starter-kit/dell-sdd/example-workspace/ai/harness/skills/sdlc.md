...
curl -s -H "Authorization: Bearer $JIRA_PAT" \
  "${JIRA_BASE_URL}/rest/api/2/issue/{ISSUE_ID}?fields=summary,description,issuetype,priority,status,labels,components,${AC_CUSTOM_FIELD},issuelinks"
```

Validate:
- Issue exists (not 404)
- Has a description (not empty)
- Has acceptance criteria (`${AC_CUSTOM_FIELD}` — field ID configured in `local.config`) or description contains testable requirements

If the issue lacks sufficient detail, report what's missing and ask the user to update the issue before continuing.
