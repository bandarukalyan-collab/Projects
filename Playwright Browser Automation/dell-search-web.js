...
// NEW CODE START: Config-driven credential handling
const configPath = path.join(__dirname, 'config.json');
let config = { credentials: { saveCredentials: false, savePath: '.env.local' } };
...
const TARGET_DOCUMENT_TITLE = config.automation?.targetDocumentTitle || 'XML-LABEL DATA ATTACHMENT-V3-ODM';
const DEBUG_SCREENSHOTS = process.env.DEBUG_SCREENSHOTS === '1';

// NEW CODE START: Accept credentials as parameters or read from env
function getCredentials(providedUsername, providedPassword) {
...
function saveCredentialsIfNeeded(username, password) {
  if (config.credentials?.saveCredentials) {
    const envContent = `DELL_USERNAME=${username}\nDELL_PASSWORD=${password}\n`;
    fs.writeFileSync(path.join(__dirname, config.credentials.savePath || '.env.local'), envContent);
...
function readKeywords(providedKeywords) {
  // NEW CODE START: Accept keywords as parameter or read from file
  if (providedKeywords && Array.isArray(providedKeywords)) {
    return providedKeywords.filter(k => k.trim());
  }
...
async function searchAndDownloadKeyword(context, page, keyword, index, total, logProgress = console.log) {
  logProgress(`Processing keyword ${index + 1}/${total}: ${keyword}`, 'info');
  ...
  logProgress('Automating search form...', 'info');

  const defaultDataSet = config.automation?.defaultDataSet || 'Manufacturing (ODM) Production';
  const dataSetSelected = await timed('Data Set selection', () => selectOptionContainingText(main, defaultDataSet));
  if (!dataSetSelected) throw new Error(`Could not select ${defaultDataSet} from Data Set.`);
  logProgress(`Selected Data Set: ${defaultDataSet}`, 'info');
  ...
  logProgress(`Filled Keyword(s): ${keyword}`, 'info');
  ...
  logProgress('Clicked Apply', 'info');

  logProgress('Waiting for search results...', 'info');
  ...
  logProgress(`Downloaded file: ${downloadedPath}`, 'success');

  // NEW CODE START: Rename downloaded file to match keyword
  const { renameDownloadedFile } = require('./file-renamer');
  const renamedPath = renameDownloadedFile(downloadedPath, keyword);
  if (renamedPath !== downloadedPath) {
    logProgress(`Renamed to: ${renamedPath}`, 'success');
  }
  // NEW CODE END
  ...
  logProgress(`Keyword ${keyword} completed`, 'success');
  return { keyword, downloadedPath: renamedPath || downloadedPath, status: 'downloaded' };
}

// NEW CODE START: Accept parameters for credentials, keywords, progress callback, and headless mode
async function openDocumentSearch(providedUsername = null, providedPassword = null, providedKeywords = null, progressCallback = null, headlessMode = true) {
  const { username, password } = getCredentials(providedUsername, providedPassword);
  
  if (!username || !password) {
    throw new Error('Username and password are required (provide as parameters or set DELL_USERNAME and DELL_PASSWORD environment variables).');
  }

  // Helper function to send progress updates
  const logProgress = (message, type = 'info') => {
    console.log(message);
    if (progressCallback) progressCallback(message, type);
  };

  const { browser, context, closeBrowser } = await getBrowserContext(headlessMode);
  const { page, shouldNavigate } = await getOrCreateDellPage(context);
  page.setDefaultTimeout(15000);
  const keywords = readKeywords(providedKeywords);
  const summary = [];

  try {
    if (shouldNavigate) {
      logProgress('Opening Dell portal...', 'info');
      await page.goto(DELL_URL, { waitUntil: 'domcontentloaded', timeout: 60000 });
    } else {
      logProgress('Dell portal is already open. Reusing current page state.', 'info');
      await page.waitForTimeout(750);
    }

    logProgress('Logging in...', 'info');
    await loginIfNeeded(page, username, password);
    logProgress('Login successful!', 'success');
    await waitForPortalShell(page);

    await saveDebugScreenshot(page, '01-before-menu-click.png');

    logProgress(`Loaded ${keywords.length} keyword(s).`, 'info');
    for (let index = 0; index < keywords.length; index += 1) {
      logProgress(`Processing keyword ${index + 1}/${keywords.length}: ${keywords[index]}`, 'info');
      const result = await searchAndDownloadKeyword(context, page, keywords[index], index, keywords.length, logProgress);
      summary.push(result);
    }

    logProgress('Batch summary:', 'info');
    for (const item of summary) {
      logProgress(`${item.keyword}: ${item.status}${item.downloadedPath ? ` -> ${item.downloadedPath}` : ''}`, 'info');
    }

    // NEW CODE START: Save credentials if configured
...
async function getBrowserContext(headlessMode = false) {
  try {
    ...
    const context = await chromium.launchPersistentContext('./chrome-user-data', {
      ...
      viewport: headlessMode ? { width: 1920, height: 1080 } : null,
